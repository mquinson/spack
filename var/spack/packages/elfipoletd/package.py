...
class Elfipoletd(Package):
    """
    A Finite Element Time Domain Electromagnetic Solver.
    ...
    try:
        ...
        version('master', git=repo+'elfipoletd.git', branch='master')
        version('1.7.0', git=repo+'elfipoletd.git', tag='v1.7.0')
    except KeyError:
        pass

    version('src')
    version('devel', git="/Users/sylvand/local/elfipoletd", branch='master')
    variant('shared', default=True, description='Build ELFIPOLETD as a shared library')
    ...
    if os.getenv("LOCAL_PATH"):
        project_local_path = os.environ["LOCAL_PATH"] + "/elfipoletd"
