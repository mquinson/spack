...
class ScotchEsmumps(Package):
    ...
    def patch(self):
        with working_dir('src/Make.inc'):
            ...
            filter_file(r'^LDFLAGS\s*=.*$', 'LDFLAGS = -lz -lm -lrt -lpthread', *makefiles)
        ...
        with working_dir('src/esmumps'):
            ...
            mf.filter('\$\(CC\) \$\(CFLAGS\) -I\$\(includedir\).*', '$(CC) $(CFLAGS) -I$(includedir) $(<) -o $(@) -L$(libdir) $(LDFLAGS) -L. -l$(ESMUMPSLIB) -l$(SCOTCHLIB) -lscotch -l$(SCOTCHLIB)errexit -lz -lm -lpthread')
