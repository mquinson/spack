    ...
    def install(self, spec, prefix):
        if os.getenv('COLAMD_DIR'):
            ...
            if os.path.isdir(colamdroot):
                ...
                raise RuntimeError(colamdroot+' directory does not exist.'+' Do you really have openmpi installed in '+colamdroot+' ?')
        else:
            raise RuntimeError('COLAMD_DIR is not set, you must set this environment variable to the installation path of your colamd')

    def setup_dependent_package(self, module, dep_spec):
        """Dependencies of this package will get the link for colamd."""
        self.spec.cc_link = "-L%s -lcolamd -lsuitesparseconfig" % self.spec.prefix.lib
        if platform.system() == 'Linux':
            self.spec.cc_link += " -lrt"