    ...
    def install(self, spec, prefix):
        if os.getenv('EZTRACE_DIR'):
            ...
            if os.path.isdir(eztraceroot):
                ...
                raise RuntimeError(eztraceroot+' directory does not exist.'+' Do you really have openmpi installed in '+eztraceroot+' ?')
        else:
            raise RuntimeError('EZTRACE_DIR is not set, you must set this environment variable to the installation path of your eztrace')
