    ...
    def install(self, spec, prefix):
        if os.getenv('LAPACK_DIR'):
            ...
            if os.path.isdir(netliblapackroot):
                ...
                raise RuntimeError(netliblapackroot+' directory does not exist.'+' Do you really have openmpi installed in '+netliblapackroot+' ?')
        else:
            raise RuntimeError('LAPACK_DIR is not set, you must set this environment variable to the installation path of your netlib-lapack')

    def setup_dependent_package(self, module, dep_spec):
        """Dependencies of this package will get the link for netlib-lapack."""
        self.spec.cc_link="-L%s -llapack -ltmglib" % self.spec.prefix.lib
