    ...
    def install(self, spec, prefix):
        if os.getenv('FFTW_DIR'):
            ...
            if os.path.isdir(fftwroot):
                ...
                raise RuntimeError(fftwroot+' directory does not exist.'+' Do you really have openmpi installed in '+fftwroot+' ?')
        else:
            raise RuntimeError('FFTW_DIR is not set, you must set this environment variable to the installation path of your fftw')
