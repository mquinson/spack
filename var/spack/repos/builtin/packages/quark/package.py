    ...
    def install(self, spec, prefix):
        if os.getenv('QUARK_DIR'):
            ...
            if os.path.isdir(quarkroot):
                ...
                raise RuntimeError(quarkroot+' directory does not exist.'+' Do you really have openmpi installed in '+quarkroot+' ?')
        else:
            raise RuntimeError('QUARK_DIR is not set, you must set this environment variable to the installation path of your quark')
